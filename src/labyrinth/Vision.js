...
LabyrinthFloor.prototype.updateFogOfWar = function() {
	this.grid.forEach2d((tile, i, j)=>tile.fowPre());
	this.party.tile.fowStand();
	this.setVisibleAdamMil(this.party, 4);
	this.grid.forEach2d((tile, i, j)=>tile.fowPost());
}
LabyrinthFloor.prototype.setVisibleAdamMil = function(origin, rangeLimit) {
	...
	//console.log("ah");
	//this.computeOctant(0, origin, rangeLimit, 1, new Slope(1, 1), new Slope(0, 1));
	//this.computeOctant(1, origin, rangeLimit, 1, new Slope(1, 1), new Slope(0, 1));
	//this.computeOctant(6, origin, rangeLimit, 1, new Slope(1, 1), new Slope(0, 1));
}

	

LabyrinthFloor.prototype.computeOctant = function(octant, origin, rangeLimit, x, top, bottom) {
	...
	for(; x <= rangeLimit; x++) {// (x <= (uint)rangeLimit) == (rangeLimit < 0 || x <= rangeLimit)
		//console.log("checking "+x+", scoped from "+top.y+"/"+top.x+" to "+bottom.y+"/"+bottom.x)
		...
		if(top.x == 1) {// if top == ?/1 then it must be 1/1 because 0/1 < top <= 1/1. this is special-cased because top
			...
			//AAAAAAAAAAAA somehow I replaced top.X with top.y ^ here and it took me three hours to fix -gPG
		...
		for (var y = topY; y >= bottomY; y--) { // use a signed comparison because y can wrap around when decremented
			if (rangeLimit < 0 || true/*this.getDistance(x, y) <= rangeLimit*/) { // skip the tile if it's out of visual range
				...
				//console.log(y, bottom.less(y*4+1, x*4-1))
				if (isVisible)
				...
				if (x < rangeLimit) { // but don't bother adjusting them if this is the last column anyway
					if (isOpaque) {
						if (wasOpaque == 0) { // if we found a transition from clear to opaque, this sector is done in this column,
							...
							var nx = x*2, ny = y*2+1; // top center by default
							...
							if (top.greater(ny, nx)) { // we have to maintain the invariant that top > bottom, so the new sector
								...
								if (y == bottomY) {
									...
									break;
								} else // don't recurse unless necessary
						...
						if(wasOpaque > 0) { // if we found a transition from opaque to clear, adjust the top vector downwards
							...
							var nx = x*2, ny = y*2+1; // the bottom of the opaque tile (oy*2-1) equals the top of this tile (y*2+1)
							// NOTE: if you're using full symmetry and want more expansive walls (recommended), comment out the next line
...
LabyrinthFloor.prototype.blocksLight = function(x, y, octant, origin) {
	//console.log(x, y, octant, origin);
	var nx = origin.gridX;
	var ny = origin.gridY;
	switch(octant) {
	...
	//return this.blocksLight(nx, ny);
	if (nx < 0 || ny < 0 || nx >= this.grid.length || ny >= this.grid[0].length)
		return true;
	//console.log(nx, ny);
	return this.grid[nx][ny].blocksLight();
}

LabyrinthFloor.prototype.setVisible = function(x, y, octant, origin) {
	//console.log(x, y);
	var nx = origin.gridX;
	var ny = origin.gridY;
	switch(octant) {
	...
	if (nx < 0 || ny < 0 || nx >= this.grid.length || ny >= this.grid[0].length)
		return false;
	//console.log("yyeah");
	this.grid[nx][ny].fowSee();
	//this.setVisible((int)nx, (int)ny);
}

LabyrinthFloor.prototype.getDistance = function(x, y) {
	return 
}
